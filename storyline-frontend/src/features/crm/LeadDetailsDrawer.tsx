  ...
  const handleAddFollowUp = async (e: React.FormEvent) => {
    ...
    try {
      await crmApi.createFollowUp({
        ...
        // using a dummy user id for now since we don't have auth context fully wired in this snippet
        performedByUserId: 1 
      });
      setNewFollowUp({ interactionType: 'CALL', notes: '' });
      // Refresh follow ups
          ...
          <>
            ...
            <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                {/* Form */}
                <form onSubmit={handleAddFollowUp} style={{ backgroundColor: 'var(--bg-main)', padding: '16px', borderRadius: '8px', marginBottom: '24px' }}>
                  <div style={{ display: 'flex', gap: '12px', marginBottom: '12px' }}>
                    ...
                    <input 
                      ...
                      style={{ flex: 1 }} 
                      placeholder="Log notes about this interaction (e.g. 'Not answered', 'Left voicemail')"
                  ...
                  {followUps.length === 0 ? (
                    ...
                    followUps.map(fu => (
                      <div key={fu.id} style={{ display: 'flex', gap: '16px' }}>
                         ...
                         <div style={{ flex: 1, backgroundColor: 'var(--bg-main)', padding: '12px 16px', borderRadius: '8px' }}>
                            ...
                            <p style={{ margin: 0, fontSize: '0.875rem', color: 'var(--text-color)' }}>{fu.notes}</p>
                         </div>
                      </div>
                    ))
                  )}
